...
const pinIcon = (score, want) => {
  ...
  return L.divIcon({ className: "", html: `<div class="pin-shadow flex h-8 min-w-8 items-center justify-center rounded-full text-[11px] font-bold text-white" style="background:${color}">${label}</div>`, iconSize: [32, 32], iconAnchor: [16, 16] });
};
            ...
            {pins.map((p) => (
              <Marker key={`${p.id}-${p.want}`} position={[p.lat, p.lng]} icon={pinIcon(p.score, p.want)}>
                <Popup>
                  ...
                  <div className="text-xs text-stone-500">{p.place.neighborhood}</div>
                </Popup>
              </Marker>
            ))}
